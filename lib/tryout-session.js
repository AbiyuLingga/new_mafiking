...
const FREE_MATH_TIME_LIMIT_SECONDS = 15 * 60;
...
function createTryoutSession({ userId, problemIds, timeLimitSeconds = FREE_MATH_TIME_LIMIT_SECONDS, now = new Date() }) {
    const startedAt = new Date(now);
    const limit = Math.max(60, Math.min(60 * 60, Number(timeLimitSeconds) || FREE_MATH_TIME_LIMIT_SECONDS));
