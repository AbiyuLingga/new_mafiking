...
function markPaymentPaid(db, {
    ...
    return db.transaction(() => {
        ...
        notifyPaymentSuccess({ db, payment: { ...payment, status: 'SUCCESS' }, grantId, source });
...
function notifyPaymentSuccess({ db, payment, grantId, source }) {
    ...
    // Publish ke SSE broadcaster (non-blocking)
    try {
        const broadcaster = require('./payment-broadcaster');
        broadcaster.publish(payment.merchant_order_id, {
            status: 'SUCCESS',
    ...
    // Kirim email (best-effort, non-blocking)
    try {
        ...
        if (!mailer || (typeof mailer.sendMail !== 'function' && typeof mailer.send !== 'function')) {
            return; // mailer not configured, skip
        }
        setImmediate(async () => {
            try {
                const mailer = require('./mailer');
                ...
                const rendered = emailTpl.renderPaymentSuccess
                    ? emailTpl.renderPaymentSuccess({ user, payment })
                    : null;
                if (!rendered) return;
                const send = mailer.sendMail || mailer.send;
                if (typeof send !== 'function') {
                    console.warn('[mailer] mailer.sendMail/send not found, skipping');
                    return;
                }
                await send({
                    to: user.email,
                ...
                `).run(payment.merchant_order_id, source || 'unknown', JSON.stringify({ to: user.email, template: 'paymentSuccess' }));
            } catch (err) {
