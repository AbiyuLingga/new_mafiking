...
function markProviderSessionExpired(provider) {
    ...
    // Best-effort: clear any internal client state.
    if ('qris' in provider) provider.qris = null;
    if ('client' in provider) provider.client = null;
}
...
class SelfHealingCollector {
    ...
    async _sendHeartbeat() {
        ...
        const result = await postHeartbeat(this.heartbeat, payload);
        this.lastHeartbeatAt = this.now();
        this.lastHeartbeatOk = Boolean(result.ok);
        if (!result.ok) {
            this.heartbeatErrors += 1;
            this.log?.warn?.(`[self-healing] heartbeat failed: ${result.error || 'unknown'}`);
        }
    }
}
