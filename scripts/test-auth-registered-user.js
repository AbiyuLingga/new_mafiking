    ...
    prepare(sql) {
        ...
        return {
            get(id) {
                ...
                if (id === 4) return { password_hash: 'none', clerk_id: 'user_clerk_123', auth_provider: 'clerk' };
                if (id === 5) return { password_hash: 'none', clerk_id: 'user_linked_123', auth_provider: 'linked' };
...
assert.strictEqual(isRegisteredUser(db, 4), true, 'Clerk users must count as logged in even without a local password');
