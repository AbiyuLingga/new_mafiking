...
function alertCollectorHealthDegraded(lastSuccessAtMs) {
    if (!Number.isFinite(lastSuccessAtMs)) return;
    const minutesSince = (Date.now() - lastSuccessAtMs) / 60000;
    if (minutesSince < 5) return;
    deliver({
        severity: minutesSince < 15 ? 'warning' : 'critical',
        title: 'Collector health degraded',
        source: 'payment.collector',
        details: { minutesSinceLastSuccess: Math.floor(minutesSince) },
    });
}

function alertHighAmbiguityRate(ratePerHour) {
    if (ratePerHour < 5) return;
    deliver({
        severity: 'warning',
        title: 'High ambiguous payment rate',
        source: 'payment.matcher',
        details: { ratePerHour },
    });
}

function alertSseFailure(merchantOrderId, error) {
    deliver({
        severity: 'warning',
        title: 'SSE payment push failed',
        source: 'payment.sse',
        details: { merchantOrderId, error: String(error || '').slice(0, 200) },
    });
}

function alertEmailFailure(merchantOrderId, error) {
    deliver({
        severity: 'warning',
        title: 'Payment success email failed',
...
module.exports = {
    ...
    alertCollectorHealthDegraded,
    alertEmailFailure,
    alertHighAmbiguityRate,
