...
const CLERK_PASSWORD_HASH = 'clerk';
...
function findUserByClerkId(db, clerkId) {
  return db.prepare('SELECT id, username, display_name, role, clerk_id, email, auth_provider, email_verified_at FROM users WHERE clerk_id = ?').get(clerkId);
}

function readUser(db, userId) {
  return db.prepare(`
    SELECT id, username, display_name, avatar_url, fakultas, phone_number, semester, jurusan, mapel_prioritas, referral_source, onboarding_completed_at, role, xp, level, badge_tier, streak_days, highest_streak, last_active, email, auth_provider, email_verified_at
    FROM users
    WHERE id = ?
  `).get(userId);
}
...
function upsertClerkUser(db, { clerkId, email = '', displayName = '', linkByEmail = true } = {}) {
    ...
    if (linkedByEmail) {
      ...
      db.prepare(`
        ...
        SET clerk_id = ?,
            ...
            auth_provider = ?,
            password_hash = CASE WHEN password_hash = 'none' THEN ? ELSE password_hash END,
            email_verified_at = COALESCE(email_verified_at, CURRENT_TIMESTAMP)
        WHERE id = ?
      `).run(clerkId, normalizedEmail, provider, CLERK_PASSWORD_HASH, linkedByEmail.id);
      return {
  ...
  const info = db.prepare(`
    INSERT INTO users (username, password_hash, display_name, role, clerk_id, email, auth_provider, email_verified_at)
    VALUES (?, ?, ?, 'user', ?, ?, 'clerk', CURRENT_TIMESTAMP)
  `).run(username, CLERK_PASSWORD_HASH, normalizedDisplay, clerkId, normalizedEmail);
...
function mergeGuestIntoUser(db, guestUserId, targetUserId) {
  ...
  const merge = db.transaction(() => {
    const guest = db.prepare(`
      ...
      FROM users
      WHERE id = ?
        AND password_hash = 'none'
        AND (clerk_id IS NULL OR clerk_id = '')
        AND COALESCE(auth_provider, 'local') = 'local'
