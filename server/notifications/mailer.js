...
function isGmailSmtpHost(host) {
  return String(host || '').trim().toLowerCase() === 'smtp.gmail.com';
}
...
function getConfig() {
  ...
  const configuredFrom = normalizeEmail(process.env.MAIL_FROM || smtpUser || 'mafikingsolusitpb@gmail.com');
  const from = isGmailSmtpHost(host) && smtpUser
    ? smtpUser
    : configuredFrom;
  if (
    ...
    && !warnedGmailFromMismatch
  ) {
    warnedGmailFromMismatch = true;
    console.warn('[mailer] Gmail SMTP forces MAIL_FROM to SMTP_USER to avoid spoof-like From headers.');
  }
  return {
    host,
